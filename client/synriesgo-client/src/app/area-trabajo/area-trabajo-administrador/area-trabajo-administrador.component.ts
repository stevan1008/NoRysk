...
import { ModulosAdministrador } from './ModulosAdministrador';

@Component({
  selector: 'app-area-trabajo-administrador',
  templateUrl: './area-trabajo-administrador.component.html',
  styleUrls: ['./area-trabajo-administrador.component.css']
})
export class AreaTrabajoAdministradorComponent implements OnInit {

  @Input() nombreUsuario: string;

  public modulosAdministrador: any;
  ...
  constructor() {
    ...
    this.modulosAdministrador = new ModulosAdministrador().getModulos();
  ...
  private obtenerInformacionModulo(modulo: number) {
    return this.modulosAdministrador.filter((x: any) => { return +x.id === modulo })[0];
  }
